...
class InstructorModel extends Model
{
        ...
        try {
            $st = $this->db->prepare(
                'INSERT INTO instructors(first_name, last_name, email)
                 VALUES (:fn, :ln, :em) RETURNING id'
            );
            $st->execute([':fn' => $i->first_name, ':ln' => $i->last_name, ':em' => $i->email]);
            return (int)$st->fetchColumn();
        } catch (\PDOException $e) {
            error_log('InstructorModel::create error '.$e->getCode().' '.$e->getMessage());
            throw $e; // Controller yakalayıp kullanıcıya düzgün mesaj gösteriyor
        }
    }
    ...
    public function update(int $id, array $data): bool
    {
        $st = $this->db->prepare(
            'UPDATE instructors
             SET first_name = :fn, last_name = :ln, email = :em, updated_at = NOW()
             WHERE id = :id'
        );
        return $st->execute([
            ':fn' => $data['first_name'],
